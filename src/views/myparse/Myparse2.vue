...
// comment 삽입.
  ...
  const getQuesArr = text => {
    const endWord = '<div style="display: none">'
    const iArr = getInxArr(text, '<div class="question">')
  ...
  const delAnswerNo = (answerArr, noArr) => {
    for (let i in answerArr) {
      let idxArr = getInxArr(answerArr[i], noArr[i])
  ...
  const arrangeQues = (quesArr, answerArr) => {
    for (let i in quesArr) {
      let inxArr = getInxArr(quesArr[i], '</div>')
  ...
  // 인덱스 찾기
  const getInxArr = (result, keyword) => {
    const iArr = []
    // 인덱스 찾기
  ...
  const getAnswerArr = (result, keyword, endWord) => {
    // 인덱스 찾기
    const iArr = getInxArr(result, keyword)
    // let i = result.indexOf(keyword)
    // while (i != -1) {
    //   iArr.push(i)
    //   i = result.indexOf(keyword, i + 1)
  ...
  // 사이에 있는 텍스트 가져오기
  const getAmongText = (text, startWord, endWord) => {
    // 앞문자로 split
